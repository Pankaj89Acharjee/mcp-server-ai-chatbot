...
const getRelevantTables = (question, schemaCache) => {
    const keywords = question.toLowerCase().split(' ');
    return schemaCache.tables.filter((table) => keywords.some(keyword => table.includes(keyword)));
};
const getSchemaForTables = async (tableNames) => {
    const schema = await connectionManager.getSchemaCache();
    return schema.tableInfo.filter((info) => tableNames.includes(info.table));
};
const Input = z.object({
    clientQuestion: z.string().optional(),
    query: z.string().optional(),
});
const langchainToolFunction = async (clientQuestion) => {
    try {
        ...
        try {
            ...
            const currentSchema = dataSource.options.schema || 'public';
        ...
        ${finalSchema.map(({ table, schema }) => `Table: ${table}\nSchema: ${schema}\n`).join('\n')}
        ...
        if (sqlMatch) {
            ...
            if (clientQuestion.toLowerCase().includes('count') || clientQuestion.toLowerCase().includes('how many')) {
                ...
                const relevantTable = tableNames.find((table) => clientQuestion.toLowerCase().includes(table.toLowerCase()) ||
...
export const langchainTool = {
    ...
    async execute(input) {
        ...
        const { clientQuestion, query } = parsed.data;
