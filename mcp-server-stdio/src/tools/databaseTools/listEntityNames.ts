 ...
 */
async function findCandidateTables(entity: keyof typeof ENTITY_CONFIG): Promise<string[]> {
    try {
        const schema = await connectionManager.getSchemaCache();
        const tables: string[] = (schema.tables || []) as string[];
 ...
 */
async function analyzeTableColumns(table: string): Promise<string[]> {
    try {
        const desc: any[] = await connectionManager.getTableSchema(table);
        const columns = desc.map(col =>
            col.name || col.column_name || col.Field || col.COLUMN_NAME
 ...
 */
async function buildOptimalQuery(table: string, entity: keyof typeof ENTITY_CONFIG, limit: number): Promise<string> {
    ...
    const findBestColumn = (preferredCols: string[]) =>
        preferredCols.find(preferred =>
            columns.some(c => c.toLowerCase() === preferred.toLowerCase())
        ) || null;
 ...
 * @type {Tool}
 */
export const listEntityNamesTool: Tool = {
    ...
    async execute(input: Record<string, unknown>) {
        ...
        let entity: keyof typeof ENTITY_CONFIG = 'users'; // Default entity
        ...
        for (const [entityName, config] of entityMappings) {
            if (config.keywords.some(keyword => queryLower.includes(keyword))) {
                entity = entityName as keyof typeof ENTITY_CONFIG;
                break;
            }
        }
