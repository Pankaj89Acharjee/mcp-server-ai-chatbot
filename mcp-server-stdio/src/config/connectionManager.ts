...
class ConnectionManager {
    ...
    public async getSqlDatabase(): Promise<SqlDatabase> {
        if (!this.sqlDatabase) {
    ...
    public async getSchemaCache(): Promise<any> {
        const now = Date.now();
    ...
    public async executeQuery(sql: string, timeout: number = 30000): Promise<any> {
    ...
    public async getTableSchema(tableName: string, timeout: number = 10000): Promise<any> {
        const timeoutPromise = new Promise((_, reject) => 
            setTimeout(() => reject(new Error('Database query timeout')), timeout)
        );
        ...
        try {
            ...
            const tableSchema = desc ? {
                tableName: (desc as any).name,
                columns: (desc as any).columns?.map((col: any) => ({
                    name: col.name,
                    type: col.type,
    ...
    public async listTables(timeout: number = 10000): Promise<string[]> {
        const dataSource = await this.getDataSource();
        const schema = (dataSource.options as any).schema || 'public';
